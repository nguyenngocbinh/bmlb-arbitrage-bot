...
def create_app(db_service: Optional[DatabaseService] = None,
    ...
    async def settings(request: Request):
        return templates.TemplateResponse(request, "settings.html", context={"profile": load_bot_profile()})
    ...
    async def dashboard(request: Request):
        ...
        opportunities = db.get_opportunities_by_session(latest_session['id'], limit=8) if latest_session else []
        running_sessions = stats.get('running_sessions') or 0
        bot_status = 'RUNNING' if running_sessions else 'IDLE'
        tracked_exchanges = [x.strip() for x in (latest_session.get('exchanges') or '').split(',') if x.strip()] if latest_session else []
        return templates.TemplateResponse(request, "dashboard.html", context={
            "stats": stats, "sessions": sessions, "recent_trades": recent_trades,
            "daily_profit": daily_profit, "hourly_profit": hourly_profit,
            "opportunities": opportunities, "latest_session": latest_session,
            "bot_status": bot_status, "tracked_exchanges": tracked_exchanges,
        })

    @app.get("/api/sessions")
    async def get_sessions(limit: int = Query(50, ge=1, le=500), offset: int = Query(0, ge=0), status: str = Query(None), symbol: str = Query(None)):
    ...
    @app.get("/api/trades")
    async def get_trades(limit: int = Query(100, ge=1, le=1000), offset: int = Query(0, ge=0), symbol: str = Query(None), buy_exchange: str = Query(None), sell_exchange: str = Query(None), start_date: str = Query(None), end_date: str = Query(None)):
        trades = db.get_all_trades(limit, offset, symbol, buy_exchange, sell_exchange, start_date, end_date)
        return {"success": True, "data": trades, "count": len(trades)}

    @app.get("/api/sessions/{session_id}/trades")
    async def get_session_trades(session_id: int, limit: int = Query(100, ge=1, le=1000), offset: int = Query(0, ge=0)):
    ...
    async def get_hourly_profit(session_id: int = Query(None), days: int = Query(7, ge=1, le=90)):
    ...
    @app.get("/api/errors")
    async def get_errors(session_id: int = Query(None), error_type: str = Query(None), limit: int = Query(100, ge=1, le=500)):
    ...
    async def run_backtest(request: BacktestRunRequest):
        ...
        if len(exchanges) < 2:
            raise HTTPException(status_code=422, detail="Backtest cần ít nhất hai sàn giao dịch")
        result = backtest_engine.run(
            symbol=recording_session['symbol'], exchanges=exchanges,
        ...
        if result.start_time is None:
            raise HTTPException(status_code=422, detail="Phiên ghi dữ liệu chưa có snapshot để chạy backtest")
